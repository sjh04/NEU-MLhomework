...
\textbf{（1）NCP以更少参数取得领先奖励。}在highway-v0场景上，NCP以7,152参数取得34.94的奖励，与拥有13,189参数的MLP（34.88）持平，且方差（1.21）显著低于MLP（3.37）、GRU（3.36）、LSTM（7.15），训练表现更稳定。其参数效率（reward/千参数 = 4.89）是MLP（2.64）的1.9倍，GRU（1.63）的3.0倍，LSTM（0.94）的5.2倍。

\textbf{（2）结构化稀疏拓扑优于全连接。}NCP与FC-LTC参数量完全相同（7,152），唯一区别是连接拓扑。在highway-v0上，NCP奖励比FC-LTC高23\%（34.94对28.41），二者方差均处于较低水平（NCP 1.21、FC-LTC 1.10）。这一结果定量证明了NCP的四层稀疏布线结构在表达能力上优于全连接设计，且并未以训练稳定性为代价。
