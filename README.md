...
**创新点 3：结构化稀疏 vs 全连接的定量验证。** 设计严格对照实验——NCP 与 FC-LTC 参数量完全相同（7,152），唯一区别是连接拓扑。highway-v0 上 NCP reward 34.94 vs FC-LTC 28.41（+23%）。首次定量证明 NCP 的生物启发稀疏拓扑优于同参数量的全连接 LTC。
...
### 基线对比（Highway-v0, 3 seeds 均值）

| 模型 | 参数量 | Reward | 碰撞率 | Reward/1K Params |
|------|--------|--------|--------|-----------------|
| Random | 1 | 9.49 ± 2.21 | 98.0% | - |
| LSTM | 26,181 | 24.62 ± 7.15 | 56.8% | 0.94 |
| FC-LTC | 7,152 | 28.41 ± 1.10 | 40.5% | 3.97 |
| GRU | 19,717 | 32.15 ± 3.36 | 41.0% | 1.63 |
| MLP | 13,189 | 34.88 ± 3.37 | **30.9%** | 2.64 |
| **NCP (Ours)** | **7,152** | **34.94 ± 1.21** | 37.9% | **4.89** |

NCP 以最少参数量（7,152）取得最高 reward（34.94）且方差最小（1.21），参数效率是 MLP 的 1.9 倍、GRU 的 3.0 倍、LSTM 的 5.2 倍。

### 进化搜索结果

搜索 15 代后发现最优布线：`inter=3, cmd=7, sf=3, if=5, rcs=0, mf=2`（仅 15 个神经元，较手工设计减少 40%）。一个有趣发现是 `rcs=0`——最优拓扑不需要命令层循环连接。

### ODE 展开数消融

| ODE Unfolds | Reward | 碰撞率 |
|-------------|--------|--------|
| 1 | 34.53 | 41.9% |
| **3** | **35.97** | 37.2% |
| 6（默认）| 26.61 | 35.1% |
...
展开数对性能影响显著（单种子实验，仅供参考），3 步可能是精度-效率的更优权衡点。
